...
- **상태**: projection/intent 변환 최소 구조 제공
  ...
  - bridge 단위 테스트에 routed projection + none-result 시나리오 추가
  - `ProjectionResult` 1차 도입 (`intent` | `none(reason)`)
    - `BridgeRuntime.projectResult()` API 추가
...
- bridge의 framework-neutral 입력 어댑터 계약 + ProjectionResult 1차 반영 완료
