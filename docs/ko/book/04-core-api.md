...
| `manifesto-runtime` | app 실행/상태/world 연동 runtime | intent 실행 요청, runtime 설정 | ActionHandle, session/branch 상태 |
| `manifesto-sdk` | canonical 공개 진입 API | schema/effects/config | runtime App 인스턴스 |
...
## 모듈 간 협업 관점
- 실행 경로(Production Runtime): `sdk -> runtime -> world -> host -> core`
...
3. `manifesto-sdk` + `manifesto-runtime`: 실제 사용 API/실행 계층 이해
